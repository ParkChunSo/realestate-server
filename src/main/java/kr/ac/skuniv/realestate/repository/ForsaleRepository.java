...
public interface ForsaleRepository extends JpaRepository<Forsale, Long> {
    ...
    @Query(value = "select f.dealType, f.housingType, f.date, function('round', avg(f.price)) from Forsale f  where f.code like concat(:code, '%') group by f.dealType, f.housingType, function('date_format', f.date, '%Y') order by f.dealType, f.housingType")
